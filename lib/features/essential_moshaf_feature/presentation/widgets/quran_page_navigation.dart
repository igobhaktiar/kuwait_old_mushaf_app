...
import 'package:quran_app/features/essential_moshaf_feature/presentation/widgets/quran_page_widget.dart' show QuranPageWidget;
...
import 'package:quran_app/features/tenReadings/presentation/cubit/tenreadings_cubit.dart'
    show CheckYourInternetConnection, ContentNotAvailableState, TenReadingCheckingForUpdates, TenReadingsCubit, TenReadingsState, TenreadingsFilesMustBeDownloadedFirstPromptState, TenreadingsStartedDownloadingAssets, UpdateAppToBenefitFromNewFeatursState;
import 'package:quran_app/features/tenReadings/presentation/dialogs/app_needs_update_dialog.dart';
import 'package:quran_app/features/tenReadings/presentation/dialogs/we_must_download_some_files_first_dialog.dart' show showPromptToInformUserThatWeMustDownloadSomeFilesFirst;
      ...
      builder: (context, state) {
        return BlocListener<TenReadingsCubit, TenReadingsState>(
          listener: (context, tenState) {
            if (context.read<EssentialMoshafCubit>().currentMoshafType == MoshafTypes.TEN_READINGS) {
            ...
            if (tenState is TenReadingCheckingForUpdates) {
              AppConstants.showToast(context, msg: context.translate.checking_for_updates);
            }
            if (tenState is TenreadingsStartedDownloadingAssets) {
              AppConstants.showToast(context, msg: context.translate.downloading_files_in_the_background);
            }
            ...
            if (tenState is CheckYourInternetConnection) {
              if (tenState.showAlertDialog) {
                showContentNotAvailableDialog(context, msg: context.translate.check_your_internet_connection);
              } else {
                AppConstants.showToast(context, msg: context.translate.check_your_internet_connection);
              }
            }
          ...
          child: PageView.builder(
            ...
            onPageChanged: (index) async {
              context.read<EssentialMoshafCubit>().navigateToPage(index + 1, jumpToPage: false);
