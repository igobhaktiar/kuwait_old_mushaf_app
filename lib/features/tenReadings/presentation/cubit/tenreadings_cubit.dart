...
import 'package:quran_app/features/tenReadings/data/models/khelafia_word_model.dart'
    show
        ...
        SingleQeraaModel;
import 'package:quran_app/features/tenReadings/data/models/last_modified_content_model.dart'
...
class TenReadingsCubit extends Cubit<TenReadingsState> {
  ...
  Future<File> loadAudioAssetPackAsFile(
      String filename, String tempFilename) async {
    const packageName = 'kw.gov.qsa.quranapp';
    // const filename = 'assets/images/example.png'; // Example asset file path in the other app
    // const tempFilename = 'example_temp.png'; // Temporary filename in your app
    // print ("5254------- Waleed OrdinaryMoshafImage filename: $filename");
    File? file = await PlatformService.getAssetFileAudio(
        packageName, filename, tempFilename);
    return file!;
  }
...
//* Methods
  ...
  void readDownloadedJsonFilesForCurrrentPage() async {
    ...
    String currentColoredImagePath =
        AppStrings.getColoredImageFileName(currentPage);
    ...
    try {
      var jsonOsoulList = await PlatformService.getAssetsJson(
          "kw.gov.qsa.quranapp", "readings_json/Osoul_$currentPage.json");
      if (jsonOsoulList != null) {
        osoul = (jsonOsoulList as List<dynamic>)
            .map((jsonElement) =>
      ...
      var jsonShwahedList = await PlatformService.getAssetsJson(
          "kw.gov.qsa.quranapp", "readings_json/Shawahed_$currentPage.json");
      if (jsonShwahedList != null) {
        shwahedDalalatGroups = (jsonShwahedList as List<dynamic>)
            .map(
      ...
      var jsonHwamishList = await PlatformService.getAssetsJson(
          "kw.gov.qsa.quranapp", "readings_json/Hawamesh_$currentPage.json");
      if (jsonHwamishList != null) {
        hwamish = (jsonHwamishList as List<dynamic>)
      ...
      var jsonQeraatWithCordinationsList = await PlatformService.getAssetsJson(
          "kw.gov.qsa.quranapp", "readings_json/Qeraat_$currentPage.json");
      if (jsonQeraatWithCordinationsList != null) {
        khelafiaWords = (jsonQeraatWithCordinationsList as List<dynamic>)
  ...
  Future<void> playQeraaFile(SingleQeraaModel qeraa) async {
    ...
    Duration? releaseAfter = await player
        .setAudioSource(AudioSource.asset("$soundsAssetsPath/$audioFile"));
    ...
    await FirebaseAnalytics.instance.logEvent(
        name: AppStrings.analytcsEventPlayQeraaMp3,
        parameters: {'qeraa': qeraa.file!});
  ...
  void deleteAllTenFilesFiles() async {
    try {
      ...
      FirebaseAnalytics.instance
          .logEvent(name: AppStrings.analytcsEventTenReadingsFilesDeleted);
    }
  }
